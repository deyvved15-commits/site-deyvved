...
export default async function MeusProdutosPage({ searchParams }: { searchParams: Promise<{ categoria?: string }> }) {
  ...
  const myProducts = productPurchases.map(p => p.product);

  return (
    <div style={{ minHeight: "100%", background: "linear-gradient(180deg, var(--navy-darkest) 0%, var(--navy-mid) 100%)" }}>

      <div className="ka-page-header">
        <div className="ka-page-eyebrow">Minha Área</div>
        <h1 className="ka-page-title">Meus <span>Produtos</span></h1>
      ...
      <div className="ka-section" style={{ padding: "38px 44px 44px" }}>
        ...
        {filteredMyCourses.length === 0 && filteredOtherCourses.length === 0 && myProducts.length === 0 && (
