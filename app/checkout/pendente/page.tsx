    ...
    const poll = async () => {
      try {
        ...
        if (data.status === "approved") {
          setApproved(true);
          setTimeout(() => router.push(courseId ? "/cursos" : "/loja"), 2000);
        }
    ...
    <div style={{
      ...
      {elapsed > 0 && (
        <p style={{ fontSize: 11, color: "rgba(255,255,255,0.25)", marginBottom: 28 }}>
          {elapsed < 60 ? `${elapsed}s aguardando` : `${Math.floor(elapsed / 60)}m ${elapsed % 60}s aguardando`}
        </p>
      )}
      ...
      <div style={{
        ...
        <p style={{ fontSize: 12, color: "rgba(255,255,255,0.60)", lineHeight: 1.8, margin: 0 }}>
          ...
          Após confirmar no seu banco, o acesso é liberado automaticamente. Pode fechar o app bancário e aguardar aqui.
        </p>
      </div>
